...
from apps.academics.models import AcademicPolicy, AcademicYear, Classroom
...
HEADER_ALIASES = {
    ...
    "is_active": {"is_active", "active", "actif"},
...
def normalize_student(row):
    ...
    gender = payload.get("gender", "").strip().upper()
    if gender:
        aliases = {
            "M": "MALE",
            "MALE": "MALE",
    ...
    payload["date_of_birth"] = (
        payload["date_of_birth"] or None
    )
    payload["admission_date"] = (
        payload["admission_date"] or None
    )
...
def normalize_guardian(row):
    ...
    language = (
        payload.get("preferred_language", "FR").strip().upper()
        or "FR"
    )

    if language in {"FRENCH", "FRANCAIS", "FRANÇAIS"}:
        language = "FR"
...
def serializer_for_entity(entity):
    ...
    try:
        ...
        raise ValueError("Type d'import inconnu.") from exc
...
def import_people(
    *,
    ...
    serializer_class, normalize, _model = serializer_for_entity(
        entity
    )
    ...
    for index, row in enumerate(rows, start=2):
        try:
            ...
            if (
                not payload.get("first_name")
                or not payload.get("last_name")
            ):
                raise ValueError(
...
def effective_threshold(level):
    ...
    if (
        level.cycle.promotion_threshold_override
        is not None
    ):
        return (
            level.cycle.promotion_threshold_override
    ...
    if entity == "enrollments":
        ...
        rows = [
            [
                ...
                (
                    item.final_average
                    if item.final_average is not None
                    else ""
                ),
...
def template_response(entity):
    ...
    try:
        headers, example = examples[entity]
    except KeyError as exc:
        raise ValueError("Type de modèle inconnu.") from exc
